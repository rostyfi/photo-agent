...
class CountTool(BaseTool):
    ...
    def execute(self, folder_path: str | None = None, args: str | None = None) -> ChatResponse:
        ...
        try:
            ...
            return ChatResponse(
                status="success", response=f"**Processed Photos:** {count}", sender="assistant", model="N/A"
            )
            ...
            return ChatResponse(
                status="error", response=f"Failed to count photos: {e!s}", sender="assistant", model="N/A"
            )
