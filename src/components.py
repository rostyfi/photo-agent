...
def build_detail_modal_content(image_path, folder, metadata, embedding=None, embedding_error=None):
    ...
    if not metadata:
        ...
        if tags:
            rows.append(html.P([html.Strong("Tags: "), ", ".join(str(t) for t in tags)], className="mb-1"))
        ...
        if image_metadata and isinstance(image_metadata, dict) and len(image_metadata) > 0:
            ...
            try:
                ...
                for key, value in sorted(image_metadata.items()):
                    if value is not None and str(value).strip():
                        rows.append(
                            html.P([html.Strong(f"{key.replace('_', ' ').title()}: "), str(value)], className="mb-1")
                        )
...
def build_similar_photos_carousel(similar_data, folder):
    ...
    items = []
    for _i, (image_path, score) in enumerate(zip(images, scores, strict=False)):
        ...
        [
            ...
            dbc.CardBody(
                [
                    ...
                    # Live processing progress bar — populated by clientside
                    # polling of /_api/process_status when /process or /status
                    # is invoked. Hidden until processing is active.
                    html.Div(
...
def build_chat_message(sender: str, content: str, error: str | None = None) -> html.Div:
    ...
    if error:
        message_content.append(dbc.Alert(error, color="danger", className="mb-0 mt-2"))
